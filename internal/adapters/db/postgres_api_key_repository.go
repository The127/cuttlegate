...
func (r *PostgresAPIKeyRepository) Create(ctx context.Context, key *domain.APIKey) error {
	_, err := r.db.ExecContext(ctx,
		...
		key.ID, key.ProjectID, key.EnvironmentID, key.Name, key.KeyHash[:], key.DisplayPrefix, key.CreatedAt,
	)
	return err
}
...
func (r *PostgresAPIKeyRepository) GetByHash(ctx context.Context, hash [32]byte) (*domain.APIKey, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`SELECT id, project_id, environment_id, name, key_hash, display_prefix, created_at, revoked_at
		 FROM api_keys WHERE key_hash = $1`,
		hash[:],
	).Scan(&key.ID, &key.ProjectID, &key.EnvironmentID, &key.Name, &hashBytes, &key.DisplayPrefix, &key.CreatedAt, &key.RevokedAt)
	if err != nil {
...
func (r *PostgresAPIKeyRepository) ListByEnvironment(ctx context.Context, projectID, environmentID string) ([]*domain.APIKey, error) {
	...
	for rows.Next() {
		var key domain.APIKey
		var hashBytes []byte
		if err := rows.Scan(&key.ID, &key.ProjectID, &key.EnvironmentID, &key.Name, &hashBytes, &key.DisplayPrefix, &key.CreatedAt, &key.RevokedAt); err != nil {
			return nil, err
		}
		copy(key.KeyHash[:], hashBytes)
