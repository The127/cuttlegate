...
func (r *PostgresAuditRepository) Record(ctx context.Context, event *domain.AuditEvent) error {
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO audit_events (id, project_id, actor_id, action, entity_type, entity_id, entity_key, environment_slug, before_state, after_state, source, occurred_at)
		...
		event.BeforeState, event.AfterState, event.Source, event.OccurredAt,
	)
	return err
}
...
func (r *PostgresAuditRepository) ListByProject(ctx context.Context, projectID string, filter domain.AuditFilter) ([]*domain.AuditEvent, error) {
	...
	query := `SELECT a.id, a.project_id, a.actor_id, COALESCE(u.email, ''), a.action,
		a.entity_type, a.entity_id, a.entity_key, a.environment_slug, a.before_state, a.after_state, a.source, a.occurred_at
		FROM audit_events a
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&e.BeforeState, &e.AfterState, &e.Source, &e.OccurredAt,
		); err != nil {
			return nil, err
		}
