...
function useBreadcrumbs(): Crumb[] {
  ...
  for (const match of matches) {
    ...
    } else if (routeId === '/_authenticated/projects/$slug/api-keys') {
      const params = match.params as { slug: string }
      crumbs.push({
        label: 'API Keys',
        to: `/projects/${params.slug}/api-keys`,
      })
...
export function Breadcrumbs() {
  ...
  return (
    <nav aria-label="Breadcrumb" className="px-4 py-2 bg-[var(--color-surface-elevated)] border-b border-[var(--color-border)]">
      <ol className="flex items-center gap-1.5 text-sm text-[var(--color-text-secondary)] min-w-0">
        {crumbs.map((crumb, i) => {
          const isFirst = i === 0
            ...
            <Fragment key={crumb.to ?? crumb.label}>
              ...
              >
                ...
                {crumb.to ? (
                  <Link
                    to={crumb.to}
                    className="truncate hover:text-[var(--color-text-secondary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] rounded"
                  >
                    {crumb.label}
                  </Link>
                ) : (
                  <span className="truncate text-[var(--color-text-primary)] font-medium">{crumb.label}</span>
                )}
              </li>
            </Fragment>
          )
...
function Separator() {
  return <span className="text-[var(--color-text-muted)] shrink-0" aria-hidden="true">/</span>
}
