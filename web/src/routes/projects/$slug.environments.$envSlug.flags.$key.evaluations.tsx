...
const REASON_BADGE_CLASS: Record<string, string> = {
  flag_disabled: 'bg-gray-100 text-gray-600 border-gray-200',
  no_match: 'bg-yellow-50 text-yellow-700 border-yellow-200',
  rule_match: 'bg-green-50 text-green-700 border-green-200',
    ...
    <div className="mt-4 bg-white border border-gray-200 rounded-lg overflow-hidden">
      ...
      {allItems.length === 0 ? (
        <div className="px-5 py-6 text-center">
          <p className="text-sm text-gray-500">{t('audit.empty_state')}</p>
        </div>
      ) : (
        <>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500">{t('audit.col_timestamp')}</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500">{t('audit.col_user')}</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500">{t('audit.col_rule')}</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500">{t('audit.col_variant')}</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500">{t('audit.col_reason')}</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500">{t('audit.col_context')}</th>
                </tr>
              </thead>
      ...
      <tr className="hover:bg-gray-50">
        ...
        <td className="px-4 py-2 text-xs font-mono text-gray-600">
          {item.matched_rule ? (
            ...
            <span className="text-gray-400">{t('audit.no_rule_matched')}</span>
          )}
        </td>
